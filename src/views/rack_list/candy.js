...
const h = 600;// window.innerHeight;
...
function initApp (el, serverList) {
    ...
    // const camera = new THREE.PerspectiveCamera(30, w / h, 1, FAR);
...
function createRack (info) {
    ...
    // 箱后侧
    let cabh = createBox(dk - 4, h, RACK_THICK, getPosition(0, h / 2 + 2, 0 - dc / 2 + 1), sideMaterial);

    // 箱顶部
    let cabTop = new THREE.Group();

    // 顶部最下面
    let topBtm = createBox(dk + 2, RACK_THICK + 2, dc, getPosition(0, h + 3, 0));
    let commonMat = getCommonMat();
    let topTop = createBox(dk + 3, RACK_THICK, dc + 2, getPosition(0, h + 6, 0), [commonMat, commonMat, topMaterial, commonMat, commonMat, commonMat]);
    ...
    // 顶部两层大方块之间的阴影效果
    const points = [];
    points.push(new THREE.Vector3(-(dk + 2) / 2, h + 5, (dc + 2) / 2));
    points.push(new THREE.Vector3((dk + 2) / 2, h + 5, (dc + 2) / 2));
...
function createError () {
    ...
    errorGroup.add(createBoxHollow(0.5, dk, dc, h));
    return errorGroup;
}
